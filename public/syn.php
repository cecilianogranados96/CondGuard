...
$sql = "SELECT tblcontacts.id, tblclients.userid,tblcontacts.title, tblclients.company,tblclients.vat, CONCAT(tblcontacts.firstname,' ',tblcontacts.lastname) as name,tblcontacts.email,tblcontacts.phonenumber FROM `tblclients`, tblcontacts WHERE tblclients.userid = tblcontacts.userid and tblcontacts.is_primary = 0; ";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    $sql = "INSERT INTO `authorized`(`authorized_id`, `condo_owner_id`, `identity`, `name`, `vehicle_plate`, `reason`, `phone`, `entry_at`, `out_at`)  VALUES (
      '".$row["id_bitacora"]. "',
      '".$row["userid"]. "',
      '',
      '".$row["nombre"]. "',
